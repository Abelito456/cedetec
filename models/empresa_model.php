...
class empresa_model extends conexion{
    ...
    function getDatosReportePostulantes($convocatoria_lab,$page,$model){
        $where=" WHERE id_conv LIKE :id_conv";
        $array=array(
            'id_conv' => '%'.$convocatoria_lab.'%',
        );
        $columns="id_conv,id_persona,estado,created_at";
        //return $this->db->select1($columns,"persona",$where,$array);
        //Users es la función que esta en el js getBusquedaUsers
        $response= $model->paginador($columns,"postulantes","DatosReportePostulantes",$page,$where,$array);
        return $response;
