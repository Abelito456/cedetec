...
class empresa extends controllers{
   ...
   $this->view->render($this, "reportes");
    ...
    public function getDatosReportePostulantes(){
        $user=session::getSession("user");
        if(null != $user){
            if("2" == $user["rol"]){
                $count=0;
                $dataFilter = null;
                $data_Estados=null;
                $data = $this->model->getDatosReportePostulantes($_POST["convocatoria_lab"],$_POST["page"],$this->page);
                if(is_array($data)){
                    $array = $data["results"];
                    foreach ($array as $key => $value) {
                        $dataConvoCandidatos=json_encode($array[$count]);
                        $response=$this->model->getDatosPersona($value["id_persona"]);
                        $dataUser=array_merge($response);
                        $dataCandidatos=json_encode($dataUser);
